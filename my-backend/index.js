...
app.post('/api/users', (req, res) => {
  const { fname, mname, lname, email, contact, address, birthday, username, password } = req.body;
  
  // SQL query to insert user into the database
  const query = 'INSERT INTO users (fname, mname, lname, email, contact, address, birthday, username, password) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
  db.query(query, [fname, mname, lname, email, contact, address, birthday, username, password], (err, results) => {
    if (err) {
    ...
    res.status(201).json({
      id: results.insertId,
      fname, mname, lname, email, contact, address, birthday, username, password
    });
  });
});
...
app.post('/api/login', (req, res) => {
  ...
  db.query('SELECT * FROM users WHERE username = ?', [username], (err, results) => {
    ...
    const token = jwt.sign({ username: user.username, id: user.id }, 'yourSecretKey', { expiresIn: '1h' });
...
app.get('/api/user/profile', (req, res) => {
  ...
  try {
    ...
    db.query('SELECT CONCAT(fname, " ", mname, " ", lname) AS name, email FROM users WHERE id = ?', [decoded.id], (err, results) => {
...
app.put('/api/users/:id/deactivate', (req, res) => {
  ...
  db.query(query, [status, userId], (err, result) => {
    ...
    res.status(200).json({ message: `User with ID ${userId} deactivated successfully` });
  });
});
...
app.put('/api/users/:id', (req, res) => {
  ...
  const query = 'UPDATE users SET fname = ?, mname = ?, lname = ?, email = ?, birthday = ?, contact = ?, address = ?, username = ?, password = ?, status = ? WHERE id = ?';
...
app.post('/api/check-user', (req, res) => {
  ...
  const query = 'SELECT *, CONCAT(lname, ", ", fname) AS fullName FROM users WHERE barcode = ?';
  ...
  db.query(query, [barcode], (err, results) => {
    ...
    if (results.length > 0) {
      ...
      const insertQuery = 'INSERT INTO timesheet (user_id, barcode, time_in, date_in) VALUES (?, ?, ?, ?)';

      db.query(insertQuery, [userId, barcode, formattedTime, formattedDate], (insertErr) => {
        if (insertErr) {
          console.error('Error inserting into timesheet:', insertErr);
          return res.status(500).json({ message: 'Error inserting into timesheet' });
        }
